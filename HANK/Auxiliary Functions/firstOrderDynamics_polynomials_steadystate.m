...
aggregateTFP = 0;
logAggregateOutput = log(exp(aggregateTFP) * (aggregateCapital ^ 0) * (aggEmployment ^ (1 - 0))/(S));
%pA=1.2*w*exp(logAggregateOutput)/(1-(1-0.15)*bbeta);
%pB=exp(logAggregateOutput)/(1-(1-0.15)*bbeta);
