...
aggregateTFP = 0;
r(-1) = (interest(-1)) -(inflation);
w = exp(aggregateTFP) * (aggregateCapital ^ aalpha) * (1 - aalpha) * (aggEmployment ^ (-aalpha));

//----------------------------------------------------------------
// Monetary Policy (# equations = 12)
//----------------------------------------------------------------

interest = ma20;
ma1 = -ssigmaR*aggregateRshock;
ma2 = ma1(-1);
ma3 = ma2(-1);
ma4 = ma3(-1);
ma5 = ma4(-1);
...
//----------------------------------------------------------------
// Equations for Inflation (# equations = 2)
//----------------------------------------------------------------

//Having Pa and Pb be seperate and 1 in the steady-state (or just normalizing one) causes the future drop to be too large relative to growth
Pa = markup * w * (logAggregateOutput) + bbeta*(1-ttheta) * (1+inflation(+1))^(-markup/(1-markup))*Pa(+1);
Pb = (logAggregateOutput) + bbeta*(1-ttheta) * (1+inflation(+1))^(-1/(1-markup))*Pb(+1);


//Using the (old?) version of the pricing function with capital in it flips the sign at the time of the rate change
//Pa = markup*(((w*(exp(aggregateTFP)*(aggregateCapital^aalpha)*(aggEmployment^(1-aalpha)))^(1/(1-aalpha)))/((1-aalpha)*(aggregateCapital^(aalpha/(1-aalpha))))) + ((r*(exp(aggregateTFP)*(aggregateCapital^aalpha)*(aggEmployment^(1-aalpha)))^(1/aalpha))/((aalpha)*(aggEmployment^((1-aalpha)/aalpha)))))+bbeta*(1-ttheta)*((inflation(+1))^(-markup/((1-markup)*aalpha*(1-aalpha))))*Pa(+1);
//PaPb = Pa/Pb;

//Directly multiplying the next term is not the correct recursive formation
//PaPb = ((markup * w * (exp(aggregateTFP) * (aggregateCapital ^ aalpha) * (aggEmployment ^ (1 - aalpha))) + bbeta*(1-ttheta) * (inflation(+1))^(-markup/(1-markup)))/((exp(aggregateTFP) * (aggregateCapital ^ aalpha) * (aggEmployment ^ (1 - aalpha))) + bbeta*(1-ttheta) * (inflation(+1))^(-1/(1-markup))))*PaPb(+1);
1+inflation = ((1 - ttheta) / (1 - ttheta*(Pa/Pb)^(1/(1 - markup))))^(1 - markup);
